...
>
> Terakhir diperbarui: 2026-09-22 (Fase 6b — Dashboard: hubungkan ke API nyata, DONE)
...
**Fase 1 (replika visual statis), Fase 6a (fondasi API client + alur auth), dan Fase 6b (Dashboard terhubung API nyata) selesai.** Frontend React + Vite + Tailwind:
- **Fase 1:** `DashboardPage` mereplikasi `docs/design/stitch-export/dashboard_paper_trading_mobile` pixel-faithful dengan token desain asli.
...
- **Dashboard (Fase 6b): `MOCK_BOTS`/saldo hardcoded Fase 1 dihapus total**, diganti `GET /wallet`/`GET /bots`/`GET /orders` sungguhan (`lib/wallet.ts`/`lib/bots.ts`/`lib/orders.ts`, baru). `Bot` type frontend diselaraskan penuh dengan enum backend (`active`/`paused`/`stopped`, sebelumnya `running`/`paused`/`stopped` rekaan Fase 1). **Field yang dikarang untuk mockup statis dihapus** (`exchange`, `strategyLabel`, PnL, win-rate, dll — backend tidak pernah mengembalikannya); `BotList` sekarang jujur menampilkan "Analitik performa belum tersedia" alih-alih angka rekaan. `BalanceCard`'s PnL diganti label dari "24 jam" (rekaan, backend tidak melacak snapshot harian) jadi "Sejak Reset" (`saldo saat ini - saldo default`, satu-satunya PnL yang bisa dihitung jujur). Tombol start/pause/stop di `BotList` sekarang benar-benar memanggil `PATCH /bots/{id}/...`. `/api/v1/portfolio` (F-PORT-01) **diputuskan tidak dibangun** untuk sekarang — cuma ada satu sumber data (paper wallet), agregasi baru tidak memberi nilai tambah sampai ada exchange live terhubung.
- **Dibuktikan lewat browser sungguhan** (Playwright/Chromium terhadap `vite dev` + backend nyata, bukan cuma unit test bermock): alur penuh register → baca OTP dari log backend nyata → verify → login → dashboard menampilkan saldo/bot nyata (bukan mock) → sesi bertahan setelah hard reload → logout → redirect proteksi rute bekerja; dan di Fase 6b: bot dibuat via API → reload menampilkan bot nyata → klik "Jalankan Bot"/"Jeda Bot" di UI benar-benar memanggil backend dan badge status berubah → Reset Saldo lewat modal benar-benar memanggil `POST /wallet/reset`.
...
- **Belum dikerjakan (Fase 6c):** wizard pembuatan bot, halaman Portofolio (`portfolio_alokasi_saldo_scr_06`) dan Detail Bot (`detail_bot_analitik_performa`) — kedua layar ini belum punya komponen/halaman sama sekali.
...
3. `cd frontend && npm install && cp .env.example .env && npm run dev` → `http://localhost:5173` — redirect otomatis ke `/login`. Alur register → verify-otp (baca kode dari terminal backend) → login → dashboard (saldo & bot **nyata** dari backend sejak Fase 6b, buat bot lewat curl langkah 8 di bawah lalu refresh untuk melihatnya) → logout, semua bisa dicoba nyata di browser.
...
Diverifikasi end-to-end pada sesi ini: lint, typecheck, unit test backend (116) + frontend (39), e2e test backend (36, melawan PostgreSQL nyata), production build backend + frontend, boot smoke test, screenshot visual dashboard (Fase 1), alur auth/market-data/exchange/paper-trading/bot/evaluate penuh via curl manual — termasuk pembuktian nyata jalur fail-safe (kill switch live trading, 503 saat exchange tak terjangkau, order ditolak aman tanpa menyentuh saldo, disconnect exchange otomatis menghentikan bot terkait, evaluate mengembalikan `MARKET_DATA_UNAVAILABLE` bukan data karangan saat Binance tak terjangkau) — dan sejak Fase 6a/6b, alur auth + dashboard frontend penuh (register→verify-otp→login→dashboard dengan data nyata→aksi start/pause bot nyata→reset saldo nyata→reload→logout) diverifikasi lewat browser sungguhan (Playwright), bukan cuma test bermock — lihat `CHANGELOG.md` untuk rincian per fase.
