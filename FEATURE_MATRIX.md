...
| F-WL-01 *(baru, dari desain)* | Whitelist Alamat Withdrawal + Timelock 24 Jam | `tambah_alamat_whitelist_24h_timelock_modal` | *(belum ada di SDD/SRS — perlu didefinisikan)* | **BLOCKED** | — | Keputusan PO (lihat konflik di `PROJECT_STATUS.md` §3.2) | Sistem tidak mengkustodi dana — apakah fitur ini masih relevan? |
...
- **TODO:** 15
- **TBD:** 1 (F-SUB-01)
- **BLOCKED:** 2 (F-WL-01 menunggu keputusan scope; Live Trading menunggu kepatuhan hukum — permanen untuk MVP)

Belum ada satu fitur pun yang diimplementasikan — proyek masih di Fase 0. Matriks ini akan diperbarui baris-per-baris saat fitur mulai dikerjakan di Fase 1 dan seterusnya.
